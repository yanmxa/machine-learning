# coding:utf-8
'''
AR : Auto Regressive自回归模型，过去若干时刻（p）的点通过线性组合，再加上白噪声就可以预测未来某个时刻的点
MA : Moving Average滑动平均模型，历史白(q)噪声进行线性组合来影响当前时刻点，与AR不同的是MA模型是历史白噪声的线性组合
ARMA: Auto Regressive Moving Average自回归滑动平均模型，是AR(p)和MA(q)的混合
ARIMA: Auto Regressive Integrated Moving Average差分自回归平均模型，
    相对于上面多了一个差分过程，作用是对不平稳数据进行差分平稳，在差分平稳后进行建模，差分阶数d
'''
